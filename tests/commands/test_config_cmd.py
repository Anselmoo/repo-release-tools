...
# ---------------------------------------------------------------------------
# cmd_config --reference (new feature)
# ---------------------------------------------------------------------------


def test_cmd_config_reference_writes_file_and_returns_0(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    """--reference writes docs/rrt-config-reference.toml, returns 0, file is valid TOML."""
    import tomllib
    ...
    rc = config_cmd.cmd_config(argparse.Namespace(reference=True, check=False, dry_run=False))

    assert rc == 0
    ref_path = tmp_path / "docs" / "rrt-config-reference.toml"
    assert ref_path.exists(), "reference file must be written"
    content = ref_path.read_text(encoding="utf-8")
    # Must be parseable by tomllib
    tomllib.loads(content)
    # Must contain the canonical header
    assert "# Generated by 'rrt config reference'" in content
    # Success output
    out = capsys.readouterr().out
    assert out  # some success output printed


def test_cmd_config_reference_creates_docs_dir(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """--reference creates docs/ directory if it doesn't exist yet."""
    monkeypatch.chdir(tmp_path)
    ...
    assert rc == 0
    assert (tmp_path / "docs" / "rrt-config-reference.toml").exists()


def test_cmd_config_reference_check_passes_when_current(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    """--reference --check returns 0 when committed file matches freshly-generated text."""
    monkeypatch.chdir(tmp_path)
    # Pre-write the reference file with the freshly-generated content
    ...
    docs_dir = tmp_path / "docs"
    docs_dir.mkdir()
    (docs_dir / "rrt-config-reference.toml").write_text(text, encoding="utf-8")

    rc = config_cmd.cmd_config(argparse.Namespace(reference=True, check=True, dry_run=False))

    assert rc == 0
    out = capsys.readouterr().out
    assert out  # success message printed


def test_cmd_config_reference_check_fails_on_drift(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    """--reference --check returns 1 when committed file differs from freshly-generated."""
    monkeypatch.chdir(tmp_path)
    docs_dir = tmp_path / "docs"
    docs_dir.mkdir()
    (docs_dir / "rrt-config-reference.toml").write_text(
        "# stale content\n[tool.rrt]\n", encoding="utf-8"
    )
    ...
    assert rc == 1
    captured = capsys.readouterr()
    # diff must be emitted to stderr
    assert captured.err


def test_cmd_config_reference_check_fails_when_missing(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    """--reference --check returns 1 when docs/rrt-config-reference.toml is missing."""
    monkeypatch.chdir(tmp_path)
    # docs/ exists but reference file does not
    (tmp_path / "docs").mkdir()
    ...
    assert err  # error reported to stderr


def test_cmd_config_reference_dry_run_writes_nothing(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    """--reference --dry-run prints plan, writes nothing, returns 0."""
    monkeypatch.chdir(tmp_path)

    rc = config_cmd.cmd_config(argparse.Namespace(reference=True, check=False, dry_run=True))

    assert rc == 0
    ref_path = tmp_path / "docs" / "rrt-config-reference.toml"
    assert not ref_path.exists(), "dry-run must not write the file"
    out = capsys.readouterr().out
    assert out  # some plan output printed


def test_register_adds_reference_flag() -> None:
    """register() exposes --reference on the config sub-parser."""
    import argparse as ap

    root_parser = ap.ArgumentParser()
    subs = root_parser.add_subparsers(dest="command")
    config_cmd.register(subs)
    parsed = root_parser.parse_args(["config", "--reference"])
    assert parsed.reference is True


def test_register_adds_check_flag() -> None:
    """register() exposes --check on the config sub-parser."""
    import argparse as ap

    root_parser = ap.ArgumentParser()
    subs = root_parser.add_subparsers(dest="command")
    config_cmd.register(subs)
    parsed = root_parser.parse_args(["config", "--reference", "--check"])
    assert parsed.check is True


def test_register_adds_dry_run_flag() -> None:
    """register() exposes --dry-run on the config sub-parser."""
    import argparse as ap

    root_parser = ap.ArgumentParser()
    subs = root_parser.add_subparsers(dest="command")
    config_cmd.register(subs)
    parsed = root_parser.parse_args(["config", "--reference", "--dry-run"])
