...
The command rewrites only the content between the
`<!-- rrt:auto:start:project-tree -->` and `<!-- rrt:auto:end:project-tree -->`
HTML anchors without touching anything else in the file. Pass `--dry-run` to
