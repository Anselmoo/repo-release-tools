...
| Module | Role |
|---|---|
| `src/repo_release_tools/workflow/hooks.py` | All `rrt-hooks` subcommands — branch/commit/changelog validators and auto-writers |
| `src/repo_release_tools/cli.py` | Argparse entrypoint and help/dispatch wiring for `rrt` |
| `src/repo_release_tools/changelog.py` | Changelog parsing, `[Unreleased]` management, conventional commit → bullet |
| `src/repo_release_tools/config/` | Config loading from `pyproject.toml` / `.rrt.toml` / `Cargo.toml` / `package.json`; model + docs/EOL helpers live under `model.py` and `docs_config.py` |
| `src/repo_release_tools/workflow/git.py` | Low-level git helpers used by command workflows |
| `src/repo_release_tools/eol/` | End-of-life and support-window checks used by `eol_check` |
| `src/repo_release_tools/version/semver.py` | Semver parsing and bump selection logic |
| `src/repo_release_tools/version/targets.py` | Read/write versions in pep621, package.json, go_version, python_version, custom regex |
| `src/repo_release_tools/state.py` | Persisted docs/lock state and health-check inputs |
| `src/repo_release_tools/integrations/skill_assets.py` | Bundled skill payloads installed by `rrt skill install` |
| `src/repo_release_tools/docs/` | Source document extraction, docs rendering/output formats, docs publishing orchestration, and Markdown helpers |
| `src/repo_release_tools/integrations/action.py` | GitHub Action input handling and execution helpers |
| `src/repo_release_tools/commands/` | `branch`, `bump`, `ci_version`, `config_cmd`, `docs_cmd`, `doctor`, `env_cmd`, `eol_check`, `git_cmd`, `init`, `skill`, `toc`, `tree` |
| `src/repo_release_tools/tools/inject.py` | Anchor-based file injection shared by `rrt tree --inject` and `rrt docs inject` |
...
| [`instructions/self-correction.instructions.md`](instructions/self-correction.instructions.md) | `**` | Keep workspace instructions current when a session reveals a missing rule or stale guidance |
