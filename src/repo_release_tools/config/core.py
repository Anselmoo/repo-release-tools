...
from .model import (
    ...
    _IGNORE_DIR_NAMES,
    _PYTHON_VERSION_VAR_RE,
    _RESERVED_BRANCH_TYPES,
...
def _load_artifact_targets(raw_targets: object) -> list[ArtifactTarget]:
    ...
    for item in raw_targets:
        ...
        if not isinstance(raw_inputs, list) or not all(isinstance(s, str) for s in raw_inputs):
            raise ValueError("artifact_targets.inputs must be a list of strings")
