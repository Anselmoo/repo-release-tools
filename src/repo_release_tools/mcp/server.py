...
def create_server(*, auth_token: str | None = None) -> FastMCP[Any]:
    ...
    mcp: FastMCP[Any] = FastMCP(
        ...
        instructions=(
            ...
            "version numbers, the changelog, branch or commit naming, [tool.rrt] config, or "
            "the .rrt/*.lock.toml state files. If you were about to run `rrt ...` in a shell, "
            "check here first.\n"
            "\n"
            "Prefer these tools over shelling out to the CLI for three concrete reasons: "
            "(1) they return typed JSON (Pydantic models) instead of ANSI-formatted human "
            "output you would have to parse; (2) arguments are passed directly, so a commit "
            "subject or branch name never goes through shell quoting — no escaping backticks, "
            "quotes, `!`, or newlines; (3) every mutating tool defaults to dry_run=True, so a "
            "preview is what you get unless you explicitly ask for a write. On the CLI the "
            "safety is a flag you must remember; here it is the default.\n"
            "\n"
            ...
            "`rrt tree --check`, `rrt toc`, `rrt changelog lint`, `rrt changelog compare`, "
            "any `--snapshot` write, and every `rrt-hooks` subcommand still require the CLI. "
            "Shell out for those; that is the correct surface, not a fallback.\n"
            "\n"
            "Safety: rrt_bump, rrt_branch_new, rrt_init_run, and rrt_publish_snapshot mutate "
            "the repository. Call each with its default dry_run=True first, show the user the "
