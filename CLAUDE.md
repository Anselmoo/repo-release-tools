...
- **Published docstrings are the product**: module docstrings under `commands/`, `workflow/`, `integrations/` and `eol/` are rendered into the docs site by `docs/publisher.py` — editing one changes the website. Every published doc must carry `## Overview`, `## Examples`, `## Caveats`, `## Related docs`, with Overview first and the last two in that order. A named constant may extend or re-title `__doc__` but must never replace it; mark a deliberately internal docstring with `rrt:docs-exempt`. Prose is register-checked (≤ 22 words/sentence per section). `rrt docs publish` enforces all of it via `docs/skeleton.py`, and `rrt docs publish --check` already runs in CI. See [Internal Contracts §6](docs/src/content/docs/reference/internal-contracts.mdx).
